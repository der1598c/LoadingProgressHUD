...
public enum LoadingProgressHUDStyle : Int {
    case light
    case dark
    case custom
}

public enum LoadingProgressHUDMaskType : Int {
    case none = 1
    case clear
    case black
    case gradient
...
public enum LoadingProgressHUDAnimationType : Int {
    case flat
    case native
}
...
//    let LoadingProgressHUDStatusUserInfoKey:String = "LoadingProgressHUDStatusUserInfoKey"
    ...
    private var fadeInAnimationDuration: TimeInterval = TimeInterval(LoadingProgressHUDDefaultAnimationDuration)
...
//    #endif
    ...
    private override init(frame: CGRect) {
        ...
        accessibilityIdentifier = "LoadingProgressHUD"
...
//    private var isInitializing = false
    ...
    private static let sharedView : LoadingProgressHUD = {
        var localInstance : LoadingProgressHUD?
        if Thread.current.isMainThread {
            if LoadingProgressHUD.isNotAppExtension {
                if let window = UIApplication.shared.delegate?.window {
                    localInstance = LoadingProgressHUD.init(frame: window?.bounds ?? CGRect.zero)
                } else {
                    localInstance = LoadingProgressHUD()
                }
            }
            else {
                localInstance = LoadingProgressHUD.init(frame: UIScreen.main.bounds)
            }
        } else {
    ...
    @objc private func controlViewDidReceiveTouchEvent(_ sender: Any?, for event: UIEvent?) {
        NotificationCenter.default.post(name: NotificationName.LoadingProgressHUDDidReceiveTouchEvent.getNotificationName(), object: self, userInfo: notificationUserInfo())
        
        if let touchLocation = event?.allTouches?.first?.location(in: self) {
            if getHudView().frame.contains(touchLocation) {
                NotificationCenter.default.post(name:
                    NotificationName.LoadingProgressHUDDidTouchDownInside.getNotificationName(), object: self, userInfo: notificationUserInfo())
            }
        }
    ...
    func notificationUserInfo() -> [String : Any]? {
        if let statusText = getStatusLabel().text {
            return [NotificationName.LoadingProgressHUDStatusUserInfoKey.rawValue: statusText]
        }
        return nil
    }
    ...
    @objc private func fadeIn(_ object: AnyObject?) {
        ...
        if getBackGroundView().alpha != 1.0 {
            NotificationCenter.default.post(name: NotificationName.LoadingProgressHUDWillAppear.getNotificationName(), object: self, userInfo: notificationUserInfo())
            ...
            let completionBlock : () -> Void = {
                ...
                NotificationCenter.default.post(name: NotificationName.LoadingProgressHUDDidAppear.getNotificationName(), object: self, userInfo: self.notificationUserInfo())
    ...
    @objc private func positionHUD(_ notification: Notification? = nil) {
        ...
        if LoadingProgressHUD.isNotAppExtension == false {
        ...
        if getControlView().superview == nil {
            if containerView != nil {
                ...
                if LoadingProgressHUD.isNotAppExtension {
                    ...
                    // If LoadingProgressHUD is used inside an app extension add it to the given view
                    if viewForExtension != nil {
    ...
    private func show(image: UIImage, status: String?, duration: TimeInterval) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            strongSelf.progress = Float(LoadingProgressHUDUndefinedProgress)
            strongSelf.cancelRingLayerAnimation()
            strongSelf.cancelIndefiniteAnimatedViewAnimation()
    ...
    private func dismissWithDelay(_ delay: TimeInterval, completion: (() -> Void)?) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            NotificationCenter.default.post(name: NotificationName.LoadingProgressHUDWillDisappear.getNotificationName(), object: nil, userInfo: strongSelf.notificationUserInfo())
    ...
    private func updateHUDFrame() {
        ...
        hudWidth = CGFloat(LoadingProgressHUDHorizontalSpacing + max(labelWidth, contentWidth) + LoadingProgressHUDHorizontalSpacing)
        
        // |-spacing-content-(labelSpacing-label-)spacing-|
        hudHeight = CGFloat(LoadingProgressHUDVerticalSpacing) + labelHeight + contentHeight + CGFloat(LoadingProgressHUDVerticalSpacing)
        if ((getStatusLabel().text != nil) && (imageUsed || progressUsed )) {
            // Add spacing if both content and label are used
            hudHeight += CGFloat(LoadingProgressHUDLabelSpacing)//8 [80]
        }
        ...
        var centerY: CGFloat
        if getStatusLabel().text != nil {
            let yOffset = max(LoadingProgressHUDVerticalSpacing, (minimumSize.height - contentHeight - CGFloat(LoadingProgressHUDLabelSpacing) - labelHeight) / 2.0)//12
        ...
        if CGFloat(progress) != LoadingProgressHUDUndefinedProgress {
        ...
        if imageUsed || progressUsed {
            if imageUsed {
                centerY = getImageView().frame.maxY + LoadingProgressHUDLabelSpacing + labelHeight / 2.0
            } else {
                centerY = getIndefiniteAnimatedView().frame.maxY + LoadingProgressHUDLabelSpacing + labelHeight / 2.0
            }
        } else {
            centerY = getHudView().bounds.midY
        }
    ...
    private func updateMotionEffect(forXMotionEffectType xMotionEffectType: UIInterpolatingMotionEffect.EffectType, yMotionEffectType: UIInterpolatingMotionEffect.EffectType) {
        ...
        effectX.minimumRelativeValue = -LoadingProgressHUDParallaxDepthPoints
        effectX.maximumRelativeValue = LoadingProgressHUDParallaxDepthPoints
        
        let effectY = UIInterpolatingMotionEffect(keyPath: "center.y", type: yMotionEffectType)
        effectY.minimumRelativeValue = -LoadingProgressHUDParallaxDepthPoints
        effectY.maximumRelativeValue = LoadingProgressHUDParallaxDepthPoints
...
extension LoadingProgressHUD {
    ...
    public class func set(defaultStyle style: LoadingProgressHUDStyle) {
    ...
    public class func set(defaultMaskType maskType: LoadingProgressHUDMaskType) {
        sharedView.defaultMaskType = maskType
    }
    
    public class func set(defaultAnimationType type: LoadingProgressHUDAnimationType) {
    ...
    public class func set(viewForExtension view: UIView) {
        LoadingProgressHUD.isNotAppExtension = false
    ...
    public class func show(withStatus status: String?) {
        show(progress: LoadingProgressHUDUndefinedProgress, status: status)
    }
...
extension LoadingProgressHUD {
    ...
    private func loadImageBundle(named imageName:String) -> UIImage? {
        let imageBundle = Bundle.init(for: LoadingProgressHUD.self)
